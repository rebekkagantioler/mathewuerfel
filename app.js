...
const elements = {
  ...
  profileSelectGrid: document.querySelector('#profileSelectGrid'), addProfileButton: document.querySelector('#addProfileButton'), openCloudProfileButton: document.querySelector('#openCloudProfileButton'), cloudProfileModal: document.querySelector('#cloudProfileModal'), closeCloudProfiles: document.querySelector('#closeCloudProfiles'), cloudSearchArea: document.querySelector('#cloudSearchArea'), cloudProfileSearch: document.querySelector('#cloudProfileSearch'), cloudProfileMatches: document.querySelector('#cloudProfileMatches'), cloudProfileSelected: document.querySelector('#cloudProfileSelected'), cloudProfilePin: document.querySelector('#cloudProfilePin'), cloudProfileMessage: document.querySelector('#cloudProfileMessage'), loadCloudProfile: document.querySelector('#loadCloudProfile'),
  ...
  gamesPipeCard: document.querySelector('#gamesPipeCard'), gamesPipeStatus: document.querySelector('#gamesPipeStatus'), gamesPipeLock: document.querySelector('#gamesPipeLock'),
...
function openCloudProfile(){
  ...
  cloudProfileMatches=[];elements.cloudProfileSearch.value='';elements.cloudProfileMatches.replaceChildren();elements.cloudSearchArea.classList.remove('hidden');elements.cloudProfileSelected.classList.add('hidden');elements.cloudProfileSelected.replaceChildren();elements.cloudProfilePin.value='';elements.cloudProfileMessage.textContent='Gib mindestens zwei Buchstaben ein, um dein Profil zu finden.';elements.cloudProfileModal.classList.remove('hidden');elements.cloudProfileSearch.focus();
}
async function searchCloudProfiles(){
  ...
  cloudProfileMatches.forEach(match=>{const button=document.createElement('button');button.type='button';button.className='cloud-profile-match';button.innerHTML=`<span>${match.profile_emoji||'🌟'}</span><strong>${escapeHtml(match.profile_name)}</strong><small>auswählen</small>`;button.addEventListener('click',()=>{cloudProfileMatches.forEach(item=>item.selected=false);match.selected=true;elements.cloudSearchArea.classList.add('hidden');elements.cloudProfileSelected.classList.remove('hidden');elements.cloudProfileSelected.innerHTML=`<span>${match.profile_emoji||'🌟'}</span><strong>${escapeHtml(match.profile_name)}</strong><small>ausgewähltes Profil</small>`;elements.cloudProfileMessage.textContent='Jetzt nur noch deinen PIN eingeben.';elements.cloudProfilePin.focus()});elements.cloudProfileMatches.append(button)})
}
...
function renderProfileHeader() {
  ...
  const pipeUnlocked=level>=5||teacherPreview;
  elements.gamesPipeCard.disabled=!pipeUnlocked;elements.gamesPipeCard.classList.toggle('locked',!pipeUnlocked);elements.gamesPipeStatus.textContent=pipeUnlocked?'Jetzt spielen':'ab Level 5';elements.gamesPipeLock.textContent=pipeUnlocked?'→':'🔒';
